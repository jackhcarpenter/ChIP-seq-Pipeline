...
do
        ...
        do
                ...
                echo "============================="
                echo ${file}"_"${lane} "= running mapping"
                ...
                -1 $workingdir/fastptemp/${file}_${lane}_R1.fastp \
                -2 $workingdir/fastptemp/${file}_${lane}_R2.fastp \
                -S $workingdir/bowtie/${file}_${lane}.sam

                ## Compress the aligned sam files to bam files
                samtools view \
                -b $workingdir/bowtie/${file}_${lane}.sam \
                > $workingdir/bowtie/${file}_${lane}.bam \

                echo ${file}"_"${lane} "= mapping complete"

                ## Organise mapped reads and index them for faster access during
                ## downstream processing

                echo ${file}"_"${lane} "= sorting"

                samtools sort \
                -@ ${SLURM_CPUS_PER_TASK} \
                -o $workingdir/bowtie/${file}_${lane}.sorted.bam \
                $workingdir/bowtie/${file}_${lane}.bam

                samtools index \
                $workingdir/bowtie/${file}_${lane}.sorted.bam

                echo ${file}"_"${lane} "= sorting complete"

                ## Run some stats on aligned/mapped reads
                bamtools stats \
                -in $workingdir/bowtie/${file}_${lane}.sorted.bam \
                > $workingdir/bowtie/${file}_${lane}.sorted.stats.txt
                ## look at the stats files to quality check the data

                echo ${file}"_"${lane} "= stats complete"
                echo "============================="
